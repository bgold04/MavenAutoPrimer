...
public FastaSequenceRetriever(File fasta) throws GenomicBase.SequenceFromFastaException, GenomicBase.SamHeaderFromDictException {
    ...
    File fastaDict ;
    GetSequenceFromFasta (File fasta) throws GenomicBase.SequenceFromFastaException, GenomicBase.SamHeaderFromDictException {
        fastaFile = fasta;
    ...
    public SAMFileHeader getSamHeaderFromDict  (File dict) throws GenomicBase.SamHeaderFromDictException {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (splitLine.size() != 5) {
                    throw new GenomicBase.SamHeaderFromDictException("Invalid sequence dictionary" + " - invalid number of fields. Should be 5, found " + splitLine.size());
                }
                for (int i = 1; i < splitLine.size(); i++) {
                    ...
                    try {
                        ...
                        throw new GenomicBase.SamHeaderFromDictException("Invalid sequence " + "dictionary" + " - invalid field in line " + lineCount + ", field " + (1 + i) + ". Value '" + splitLine.get(i) + "' is not an integer.", ex);
                    }
                }
