...
public class FastaSequenceRetriever {
    ...
    File fastaDict ;
    GetSequenceFromFasta (File fasta) throws SequenceFromFastaException, SamHeaderFromDictException {
        fastaFile = fasta;
        fastaDict = new File(fasta.getName() + ".fai");
        if (! fastaDict.exists()) {
            throw new SequenceFromFastaException("Fasta dictionary " + fastaDict.getAbsolutePath() + " for fasta file " + fastaFile.getAbsolutePath() + " does not exist.");
        }
        fastaRef = ReferenceSequenceFileFactory.getReferenceSequenceFile(fasta);
    }
    public String retrieveSequence(String chrom, Integer start, Integer end, String strand) throws SequenceFromFastaException {
        try {
            ...
            throw new SequenceFromFastaException("Error retrieving fasta sequence " + "from " + fastaFile.getName(), ex);
        }
    }
    public SAMFileHeader getSamHeaderFromDict  (File dict) throws SamHeaderFromDictException {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                //check all but chrom field are integers
                    try {
                        ...
                        throw new SamHeaderFromDictException("Invalid sequence " + "dictionary" + " - invalid field in line " + lineCount + ", field " + (1 + i) + ". Value '" + splitLine.get(i) + "' is not an integer.", ex);
                    }
                }
            ...
            throw new SamHeaderFromDictException("IO error reading sequence " + "dictionary file " + dict.getName(), ex);
        }
