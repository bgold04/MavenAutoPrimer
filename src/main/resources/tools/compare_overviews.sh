...
extract_overview() {
    ...
    grep -nE 'public void|public static void|class |public class|private boolean|public boolean|public LinkedHashSet|private LinkedHashSet|public static|private void|private ArrayList|protected' "$input_file" > "$output_file"
    echo "Overview saved to $output_file"
}

# Generate overviews
overview1="$work_dir/overview_MavenAutoPrimer.java.txt"
overview2="$work_dir/overview_AutoPrimer3.java.txt"
extract_overview "$file1" "$overview1"
extract_overview "$file2" "$overview2"

# Generate two-column comparison
comparison_file="$work_dir/comparison_AutoPrimer3_overview.txt"
echo "Creating two-column comparison..."
